...
for idx, (case_name, results) in enumerate(all_cases_results.items()):
    ...
    for patch in box['boxes']:
        patch.set_facecolor('powderblue')  # Set all boxes to light blue
        patch.set_edgecolor('darkcyan') 
    
    # Optionally, customize whiskers, fliers, caps, and the median line
    for whisker in box['whiskers']:
        whisker.set(color ='darkcyan', linewidth = 1)
    for cap in box['caps']:
        cap.set(color ='darkcyan', linewidth = 1)
    for median in box['medians']:
        median.set(color ='darkcyan', linewidth = 1)
